...
cursor.execute("CREATE TABLE IF NOT EXISTS characters (id INTEGER NOT NULL, simplified TEXT)")
cursor.execute("CREATE TABLE IF NOT EXISTS pinyin (id INTEGER NOT NULL, pinyin TEXT)")
cursor.execute("CREATE TABLE IF NOT EXISTS definitions (id INTEGER NOT NULL, definition TEXT)")
cursor.execute("CREATE TABLE IF NOT EXISTS cn_sentence (id INTEGER NOT NULL, cn_sentence TEXT)")
cursor.execute("CREATE TABLE IF NOT EXISTS en_sentence (id INTEGER NOT NULL, en_sentence TEXT)")
...
with open("cedict_tatoeba_userdict.txt", encoding = "utf-8") as tatoeba:
    ...
    for line in tatoeba:
        ...
        for word in definitions:
            cursor.execute("INSERT INTO definitions VALUES (?, ?)", (wordid, word.rstrip()))
        ...
        if(len(definitions_sentences) > 1):
            ...
            cursor.execute("INSERT INTO cn_sentence VALUES (?, ?)", (wordid, cn_sentence))
            cursor.execute("INSERT INTO en_sentence VALUES (?, ?)", (wordid, eng_sentence))
        wordid = wordid + 1
connection.commit()
#endregion
...
cursor2.execute("CREATE TABLE IF NOT EXISTS characters (id INTEGER NOT NULL, simplified TEXT)")
cursor2.execute("CREATE TABLE IF NOT EXISTS pinyin (id INTEGER NOT NULL, pinyin TEXT)")
cursor2.execute("CREATE TABLE IF NOT EXISTS definitions (id INTEGER NOT NULL, definition TEXT)")
